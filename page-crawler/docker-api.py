...
IMAGE_NAME = 'ecs289m/page-crawler'
...
def run_containers():
    ...
    categories = pd.read_sql('similarweb', con=engine)['Category'].unique()

    for c in categories:
        client.containers.run(IMAGE_NAME, ['python', 'main.py', c], shm_size="512M", remove=True, detach=True)
